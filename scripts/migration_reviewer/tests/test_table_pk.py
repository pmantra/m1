...
def test_pk_rule(migration_content: str, expected_result: str) -> None:
    """Test the atomicity rule."""
    # Test data
    migration_files = [{"file": "test_migration.py", "content": migration_content}]
    other_changes = []

    with open("rules/table_pk.txt", "r") as f:
        rules = {"table_pk.txt": f.read()}

    # Run validation
    results = validate_migrations(migration_files, other_changes, rules)

    assert len(results) == 1
    assert results[0]["rule"] == "table_pk.txt"
