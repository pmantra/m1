"""
Backfill CareAdvocateMemberTransitionTemplate model
"""
...
def _backfill_ca_member_transition_templates():  # type: ignore[no-untyped-def] # Function is missing a return type annotation
    templates_to_create = [
        (
            "Farewell message template",
            MessageFormats.FAREWELL,
            CareAdvocateMemberTransitionSender.OLD_CX,
        ),
        (
            "Intro followup message template",
            MessageFormats.FOLLOWUP_INTRO,
            CareAdvocateMemberTransitionSender.NEW_CX,
        ),
        (
            "Hard intro message template",
            MessageFormats.HARD_INTRO,
            CareAdvocateMemberTransitionSender.NEW_CX,
        ),
        (
            "Soft intro message template",
            MessageFormats.SOFT_INTRO,
            CareAdvocateMemberTransitionSender.NEW_CX,
        ),
        (
            "ECP farewell message template",
            MessageFormats.ECPFAREWELL,
            CareAdvocateMemberTransitionSender.OLD_CX,
        ),
        (
            "ECP intro followup message template",
            MessageFormats.FOLLOWUP_ECPINTRO,
            CareAdvocateMemberTransitionSender.NEW_CX,
        ),
        (
            "UCP farewell message template",
            MessageFormats.UCPFAREWELL,
            CareAdvocateMemberTransitionSender.OLD_CX,
        ),
        (
            "UCP intro followup message template",
            MessageFormats.FOLLOWUP_UCPINTRO,
            CareAdvocateMemberTransitionSender.NEW_CX,
        ),
    ]

    for message_description, message_format, sender in templates_to_create:
        message_type = message_format.name
        __, message_template = message_format.value
        message_body = message_template.template

        template = CareAdvocateMemberTransitionTemplate(
    ...
    db.session.commit()
    print("*** Backfilled ca member transition templates ***")


def _rewind_backfill():  # type: ignore[no-untyped-def] # Function is missing a return type annotation
    CareAdvocateMemberTransitionTemplate.query.delete()

    db.session.commit()
    print("*** Rewound ca member transition templates ***")
...
if __name__ == "__main__":
    ...
    with create_app().app_context():
        if args.get("--rewind"):
            ...
            _backfill_ca_member_transition_templates()
