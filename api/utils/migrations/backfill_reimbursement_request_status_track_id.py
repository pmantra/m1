...
def fix_track_id(action, force):  # type: ignore[no-untyped-def] # Function is missing a type annotation
    log.info(
        "Fixing change_reimbursement_request_status action track id.",
        action_id=action.id,
    )
    ...
    if len(pp) != 1:
        ...
        return
    p = pp[0]
    if p.id == action.data["track_id"]:
        log.info("Action had correct program id.")
        return
    log.info(
        "Found action care program id.",
        action_id=action.id,
        old_track_id=action.data["track_id"],
        care_program_id=p.id,
    )
    if force:
        action.data = {
            k: p.id if k == "track_id" else v for k, v in action.data.items()
        }
        db.session.commit()
