...
def backfill_in_batches(batch_size: int = 1_000):  # type: ignore[no-untyped-def] # Function is missing a return type annotation
    for batch in create_batch(batch_size=batch_size):
        log.info("Setting `accepted` value")
        session = db.session().using_bind("default")
        session.execute(
            """
            UPDATE `agreement_acceptance`
            SET `accepted` = 1
            WHERE `id` IN :ids;
            """,
            params={"ids": batch},
...
def create_batch(batch_size: int = 1_000):  # type: ignore[no-untyped-def] # Function is missing a return type annotation
    log.info("Fetching AgreementAcceptance...")

    query = db.session.query(AgreementAcceptance.id).filter(
        AgreementAcceptance.accepted.is_(None)
    )

    log.info("Building batch AgreementAcceptance data...")
    batch_num = 0
    ...
    while batch:
        ...
        to_add = [aa_id[0] for aa_id in batch]
        ...
        batch = query.filter(AgreementAcceptance.id > last_id).limit(batch_size).all()
...
def backfill(dry_run: bool = False, batch_size: int = 1_000):  # type: ignore[no-untyped-def] # Function is missing a return type annotation
    log.info(
        "Running AgreementAcceptance backfill.",
        dry_run=dry_run,
    )
