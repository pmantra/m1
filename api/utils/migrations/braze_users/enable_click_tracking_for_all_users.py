...
def enable_braze_click_tracking_for_all_users():  # type: ignore[no-untyped-def] # Function is missing a return type annotation
    log.info("Enable braze click tracking")
    braze_batch_size = 75
    ...
    for users in paginate(
        ...
        braze_user_attributes = [
            client.BrazeUserAttributes(
                ...
                attributes={"email_click_tracking_disabled": False},
            )
            for u in users
            if u.esp_id
        ]
    ...
    log.info("Enable braze click tracking completed")


if __name__ == "__main__":
    with create_app().app_context():
        enable_braze_click_tracking_for_all_users()
