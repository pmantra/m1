...
def main() -> None:
    """
    Job to download maven pharmacy direct payment shipped file from SMP SFTP server.
    """
    args = _generate_args()
...
def _generate_args():  # type: ignore[no-untyped-def] # Function is missing a return type annotation
    ...
    parser.add_argument(
        ...
        required=True,
        help="True will download from SMP the shipped file and print the rows of data to be processed,"
...
def execute_job(dry_run: bool) -> bool:
    ...
    processor = ShippedFileProcessor(dry_run=dry_run)
