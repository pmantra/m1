...
def main() -> None:
    """
    Job to download maven pharmacy direct payment schedule file from SMP SFTP server.
    """
    args = _generate_args()
...
def _generate_args():  # type: ignore[no-untyped-def] # Function is missing a return type annotation
    ...
    parser.add_argument(
        ...
        required=True,
        help="True will download from SMP and print rows of data to be processed,"
...
def execute_job(dry_run: bool) -> bool:
    ...
    processor = ScheduledFileProcessor(dry_run=dry_run)
