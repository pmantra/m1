...
from models.questionnaires import ASYNC_ENCOUNTER_QUESTIONNAIRE_OID, Questionnaire
...
class MPracticeQuestionnaireRepository(BaseRepository[QuestionnaireV2]):
    model = QuestionnaireV2
    ...
    def __init__(
        ...
        queries = query_utils.load_queries_from_file(
            "clinical_documentation/repository/queries/questionnaire.sql"
        )
        if len(queries) == 0:
        ...
        self._get_questionnaire_list_by_practitioner_query = queries[2]
    ...
    def get_recorded_answer_set(
        ...
        if row is None:
            return None
        return RecordedAnswerSetV2(**row)
    ...
    @ddtrace.tracer.wrap()
    def get_questionnaire_list_by_practitioner(
        self, practitioner_id: int
    ) -> List[QuestionnaireV2]:
        rows = self.session.execute(
            self._get_questionnaire_list_by_practitioner_query,
            {
    ...
    @ddtrace.tracer.wrap()
    def get_questionnaire_by_oid(self, oid: str) -> QuestionnaireV2 | None:
        row = self.session.execute(
    ...
    def get_question_sets_by_questionnaire_id(
        ...
        if rows is None:
            return []
        return [QuestionSetV2(**row) for row in rows]
    ...
    def get_questions_by_question_set_ids(
        ...
        if rows is None:
            return []
        return [QuestionV2(**row) for row in rows]

    @ddtrace.tracer.wrap()
    def get_answers_by_question_ids(self, question_ids: List[int]) -> List[AnswerV2]:
        if not question_ids:
            return []
        ...
        if rows is None:
            return []
        return [AnswerV2(**row) for row in rows]
    ...
    def get_provider_addenda(
        ...
        if rows is None:
            return []
        return [ProviderAddendumV2(**row) for row in rows]

    @ddtrace.tracer.wrap()
    def get_provider_addenda_answers(
        self, addendum_ids: List[int]
    ) -> List[ProviderAddendumAnswerV2]:
        if not addendum_ids:
            return []
        rows = self.session.execute(
        ...
        if rows is None:
            return []
        return [ProviderAddendumAnswerV2(**row) for row in rows]
    ...
    ) -> [RecordedAnswerV2]:  # type: ignore[valid-type] # Bracketed expression "[...]" is not valid as a type
        return [RecordedAnswerV2(**dict(row)) for row in rows]
