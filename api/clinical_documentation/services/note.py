...
from clinical_documentation.models.translated_note import (
    ...
    TranslatedAnswerV2,
    TranslatedPostAppointmentNote,
    TranslatedPostAppointmentNotes,
...
class ClinicalDocumentationNoteService:
    def __init__(
        self,
        session: ScopedSession | None = None,
        post_appointment_note_repo: PostAppointmentNoteRepository | None = None,
        questionnaire_repo: MPracticeQuestionnaireRepository | None = None,
        include_soft_deleted_question_sets: bool | None = False,
    ):
        self.session = session or db.session
        self.post_appointment_note_repo = (
            post_appointment_note_repo
            or PostAppointmentNoteRepository(session=self.session)
        )
    ...
    @ddtrace.tracer.wrap()
    def get_post_appointment_notes(
        self, appointment_ids: List[int], role: str
    ) -> TranslatedPostAppointmentNotes:
        post_appointment_note_list = self._get_translated_post_appointment_note_list(
            appointment_ids=appointment_ids, role=role
        )
        return TranslatedPostAppointmentNotes(
            post_appointment_notes=post_appointment_note_list
        )

    @ddtrace.tracer.wrap()
    def _translate_post_appointment_notes(
        self, notes: List[PostAppointmentNote], role: str
    ) -> List[TranslatedPostAppointmentNote]:
        translated_post_appointment_notes = []

        for note in notes:
            translated_post_appointment_notes.append(
                TranslatedPostAppointmentNote(
                    id=note.id,
                    appointment_id=obfuscate_appointment_id(note.appointment_id)  # type: ignore[arg-type] # Argument "appointment_id" to "TranslatedPostAppointmentNote" has incompatible type "Optional[int]"; expected "int"
                    if note.appointment_id
                    else None,
                    content=self._get_content(note, role),
                    draft=note.draft,
        ...
        return translated_post_appointment_notes

    @ddtrace.tracer.wrap()
    def _get_translated_post_appointment_note_list(
        self, appointment_ids: List[int], role: str
    ) -> List[TranslatedPostAppointmentNote]:
        notes = self.post_appointment_note_repo.get_post_appointment_notes_by_appointment_ids(
            appointment_ids=appointment_ids
        )

        if not notes:
            log.warn(
                f"Post appointment notes of appointments {appointment_ids} does not exist"
            )
            return []
        return self._translate_post_appointment_notes(notes=notes, role=role)

    @ddtrace.tracer.wrap()
    def _get_content(self, note: PostAppointmentNote, role: str) -> str:
        if note.draft and role.upper() == Role.MEMBER.name:
            return ""
        else:
            return note.content
    ...
    def get_and_translate_question_sets(
        ...
        for question_set in question_sets:
            translated_question_set = TranslatedQuestionSetV2(**vars(question_set))
    ...
    ) -> List[TranslatedQuestionV2]:
        result = []
        questions = question_set_id_to_questions.get(question_set_id, [])
        for question in questions:
            translated_question = TranslatedQuestionV2(**vars(question))
    ...
    ) -> List[TranslatedAnswerV2]:
        answers = question_id_to_answers.get(question_id, [])
        return [TranslatedAnswerV2(**vars(answer)) for answer in answers]

    @ddtrace.tracer.wrap()
    def translate_questionnaire(
        self,
        questionnaire: QuestionnaireV2,
        translated_question_sets: List[TranslatedQuestionSetV2],
        trigger_answer_ids: List[int],
    ) -> TranslatedQuestionnaireV2:
        translated_questionnaire = TranslatedQuestionnaireV2(**vars(questionnaire))
    ...
    def translate_provider_addenda(
        self, provider_addenda: List[ProviderAddendumV2]
    ) -> List[TranslatedProviderAddendumV2]:
    ...
    @ddtrace.tracer.wrap()
    def get_structured_internal_notes(
        self, appointment_id: int, practitioner_id: int
        ...
        if not questionnaire:
            log.warn(
                "No questionnaire found for appointment", appointment_id=appointment_id
            )
            return None
    ...
    def get_questionnaire(
        ...
        questionnaires = self.questionnaire_repo.get_questionnaire_list_by_practitioner(
            practitioner_id=practitioner_id
        )
        if questionnaires:
    ...
    def translate_recorded_answers(
        self,
        recorded_answers: List[RecordedAnswerV2],
        appointment_id_from_recorded_answer_set: int | None,
    ) -> List[TranslatedRecordedAnswerV2]:
        result = []
    ...
    def translate_recorded_answer_set(
        ...
        translated_recorded_answers: List[TranslatedRecordedAnswerV2],
    ) -> TranslatedRecordedAnswerSetV2:
