...
with models.DAG(
    dag_id="accumulation_data_sourcing_dag",
    params={
        "team_ns": "payments_platform",
        "service_ns": "payer_accumulation",
        "payers": [
            job["payer"] for job in accumulation_jobs if "data_sourcing" in job["jobs"]
        ],
        "run_off_schedule": False,
    },
    schedule="0 23,16 * * *",  # Combined schedule to cover all payers
    catchup=False,
    ...
    base_task_id = "accumulation_data_sourcing_job"
    ...
    get_launchdarkly_config = KubernetesPodOperator(
        ...
        cmds=[
            "python3",
            "-c",
            "from airflow.scripts.payments_platform.accumulation_helper_scripts import get_consolidated_accumulation_data_sourcing_feature_flag; "
            "get_consolidated_accumulation_data_sourcing_feature_flag()",
        ],
        do_xcom_push=True,
        **kpo_constants,
    )
    ...
    data_sourcing_jobs = [
        job for job in accumulation_jobs if "data_sourcing" in job["jobs"]
    ]
    default_payers = [job["payer"] for job in data_sourcing_jobs]
    for job in data_sourcing_jobs:
        condition_task = ShortCircuitOperator(
            ...
            op_kwargs={
                "payer": job["payer"],
                "schedule": job["job_schedules"]["data_sourcing"].value,
                "job_type": "data_sourcing",
                "default_payers": default_payers,
            },
        )

        command = f"from airflow.scripts.payments_platform.accumulation_data_sourcing import {job['payer']}_accumulation_data_sourcing_job; {job['payer']}_accumulation_data_sourcing_job()"
        data_sourcing_task = KubernetesPodOperator(
        ...
        get_launchdarkly_config >> condition_task >> data_sourcing_task
