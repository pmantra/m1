...
with models.DAG(
    dag_id="accumulation_process_responses_dag",
    params={
        ...
        "payers": [
            ...
            if "process_responses" in job["jobs"]
        ],
        "run_off_schedule": False,
    },
    schedule="0 4,19 * * *",  # Combined schedule to cover all payers
    catchup=False,
    ...
    base_task_id = "accumulation_process_responses_job"
    ...
    get_launchdarkly_config = KubernetesPodOperator(
        ...
        cmds=[
            "python3",
            "-c",
            "from airflow.scripts.payments_platform.accumulation_helper_scripts import get_consolidated_accumulation_process_responses_feature_flag; "
            "get_consolidated_accumulation_process_responses_feature_flag()",
        ],
        do_xcom_push=True,
        **kpo_constants,
    )
    ...
    process_responses_jobs = [
        job for job in accumulation_jobs if "process_responses" in job["jobs"]
    ]
    default_payers = [job["payer"] for job in process_responses_jobs]
    for job in process_responses_jobs:
        condition_task = ShortCircuitOperator(
            ...
            op_kwargs={
                "payer": job["payer"],
                "schedule": job["job_schedules"]["process_responses"].value,
                "job_type": "process_responses",
                "default_payers": default_payers,
            },
        )

        command = f"from airflow.scripts.payments_platform.accumulation_process_responses import {job['payer']}_accumulation_process_responses_job; {job['payer']}_accumulation_process_responses_job()"
        process_responses_task = KubernetesPodOperator(
        ...
        get_launchdarkly_config >> condition_task >> process_responses_task
