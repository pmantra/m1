...
from wallet.migrations.config.backfill_org_ids_on_plans import ALEGEUS_PLAN_TO_ORG_ID

log = logger(__name__)


def update_org_ids(org_to_plan_name: dict[str, int], dry_run=True):
    tot_affected_count = 0
    for i, (alegeus_plan_id, organization_id) in enumerate(org_to_plan_name.items()):
        log.info(
            "Update started",
...
def update_data(dry_run: bool = True):
    log.info("Begun.")
    try:
        update_org_ids(ALEGEUS_PLAN_TO_ORG_ID, dry_run=dry_run)
