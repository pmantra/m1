...
from marshmallow_v1 import Schema, fields, utils, MarshalResult, UnmarshalResult
from marshmallow_v1.exceptions import (
    MarshallingError,
    UnmarshallingError,
    ValidationError,
)
from marshmallow_v1.compat import unicode, OrderedDict
...
class TestSchemaValidator:
    ...
    def test_errors_are_cleared_after_loading_collection(self):
        ...
        schema = MySchema()
        _, errors = schema.load([{"foo": "bar"}, {"foo": "baz"}], many=True)
...
class TestNestedSchema:
    ...
    def test_serializer_meta_with_nested_fields(self, blog, user):
        ...
        assert data["collaborators"] == [
            UserSchema().dump(c).data for c in blog.collaborators
        ]
...
class TestSelfReference:
    ...
    def test_multiple_nested_self_fields(self, user):
        class MultipleSelfSchema(Schema):
            ...
            rels = fields.Nested("self", only="name", many=True, attribute="relatives")
...
class TestSkipMissingOption:
    ...
    def test_missing_values_are_skipped_with_many(self):
        users = [
            User(name="Joe", email=None, age=None),
