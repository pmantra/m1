...
from marshmallow_v1 import fields, utils, Schema
from marshmallow_v1.exceptions import UnmarshallingError, ValidationError
from marshmallow_v1.compat import text_type, total_seconds
...
class TestFieldDeserialization:
    ...
    def test_field_deserialization_with_custom_error_message(self):
        field = fields.String(
            validate=lambda s: s.lower() == "valid", error="Bad value."
        )
        with pytest.raises(UnmarshallingError) as excinfo:
            field.deserialize("invalid")
...
class TestUnMarshaller:
    ...
    def test_strict_mode_many(self, unmarshal):
        users = [{"email": "foobar"}, {"email": "bar@example.com"}]
        with pytest.raises(UnmarshallingError) as excinfo:
    ...
    def test_deserialize_many(self, unmarshal):
        users_data = [{"name": "Mick", "age": "71"}, {"name": "Keith", "age": "70"}]
        fields_dict = {
            "name": fields.String(),
            "age": fields.Integer(),
        }
    ...
    def test_deserialize_fields_with_attribute_param(self, unmarshal):
        data = {"username": "mick@stones.com", "name": "Mick"}
        fields_dict = {
            "username": fields.Email(attribute="email"),
    ...
    def test_preprocessing_many(self, unmarshal):
        ...
        result = unmarshal.deserialize(
            data, fields_dict, preprocess=[preprocessor], many=True
        )
...
FIELDS_TO_TEST = [
    f for f in ALL_FIELDS if f not in [fields.Enum, fields.FormattedString]
]
