...
from marshmallow_v1 import validate, utils, class_registry
from marshmallow_v1.base import FieldABC, SchemaABC
from marshmallow_v1.compat import text_type, iteritems, total_seconds, basestring
...
def _call_and_store(
    getter_func, data, field_name, field_obj, errors_dict, exception_class, strict=False
):
    ...
    try:
        ...
        if strict:
            ...
            raise err
        # Warning: Mutation!
        if hasattr(err, "underlying_exception") and isinstance(
            err.underlying_exception, ValidationError
        ):
        ...
        if isinstance(field_obj, type) and issubclass(field_obj, FieldABC):
            msg = (
                'Field for "{0}" must be declared as a '
                "Field instance, not a class. "
                'Did you mean "fields.{1}()"?'.format(field_name, field_obj.__name__)
            )
            raise TypeError(msg)
        raise
    return value
...
class Unmarshaller(object):
            ...
            try:
                ...
                if strict:
                    raise UnmarshallingError(
                        err, field=field_obj, field_name=field_name
                    )
                if isinstance(err.messages, (list, tuple)):
                    ...
                    if isinstance(self.errors.get(field_name), dict):
                        self.errors[field_name].setdefault("_schema", []).extend(
                            err.messages
                        )
                    else:
    ...
    def deserialize(
        ...
        if data is not None:
            ...
            for attr_name, field_obj in iteritems(fields_dict):
                ...
                try:
                    ...
                    msg = "Data must be a dict, got a {0}".format(
        ...
        if validators:
            ret = self._validate(
                validators, ret, fields_dict=fields_dict, strict=strict
            )
        if postprocess:
...
class Field(FieldABC):
    ...
    def __init__(
        ...
        **metadata
        ...
        if error:
            warnings.warn(
                "The error parameter is deprecated. Raise a "
    ...
    def __repr__(self):
        return (
            ...
            "validate={self.validate}, required={self.required})>".format(
    ...
    def schema(self):
        ...
        if not self.__schema:
            if isinstance(self.nested, SchemaABC):
                ...
                self.__schema = self.nested(
                    many=self.many, only=only, exclude=self.exclude
                )
...
class Number(Field):
    ...
    def __init__(
        self, default=0.0, attribute=None, as_string=False, error=None, **kwargs
    ):
        self.as_string = as_string
        super(Number, self).__init__(
            default=default, attribute=attribute, error=error, **kwargs
        )
...
class Fixed(Number):
    ...
    def __init__(
        self, decimals=5, default="0.000", attribute=None, error=None, *args, **kwargs
    ):
        warnings.warn(
        ...
        super(Fixed, self).__init__(
            default=default, attribute=attribute, error=error, *args, **kwargs
        )
...
class Url(ValidatedField):
    ...
    def __init__(self, default=None, attribute=None, relative=False, *args, **kwargs):
        super(Url, self).__init__(default=default, attribute=attribute, *args, **kwargs)
        self.relative = relative
        # Insert validation into self.validators so that multiple errors can be
        # stored.
        self.validators.insert(
            0, validate.URL(relative=self.relative, error=getattr(self, "error"))
        )
    ...
    def _validated(self, value, exception_class):
        if value not in self.choices:
            raise exception_class(
                ...
                or "{0!r} is not a valid choice for this field.".format(value)
            )
        return value
