...
from marshmallow_v1 import base, fields, utils, class_registry
from marshmallow_v1.compat import (
    with_metaclass,
    iteritems,
    text_type,
    binary_type,
...
from marshmallow_v1.orderedset import OrderedSet
...
class BaseSchema(base.SchemaABC):
    ...
    def __init__(
        ...
        self._marshal = fields.Marshaller(prefix=self.prefix)
        ...
        if self.obj is not None:
            warnings.warn(
                "Serializing objects in the Schema constructor is a "
    ...
    def data(self):
        ...
        """
        warnings.warn(
            "Accessing data through Schema.data is deprecated. "
    ...
    def _do_load(self, data, many=None, postprocess=True):
        ...
        if self.__validators__:
            validators = [partial(func, self) for func in self.__validators__]
        else:
            validators = []
        if self.__preprocessors__:
            preprocessors = [partial(func, self) for func in self.__preprocessors__]
        else:
            preprocessors = []
    ...
    def __filter_fields(self, field_names, obj, many=False):
        ...
        obj_marshallable = utils.to_marshallable_type(obj, field_names=field_names)
        if obj_marshallable and many:
            ...
            obj_dict = utils.to_marshallable_type(
                obj_prototype, field_names=field_names
            )
        else:
