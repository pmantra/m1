        ...
        if data.get("id"):
            ...
            if not user_data:
                raise ValidationError(
                    "Must provide user when creating a new todo item."
                )
            user = User.get(User.email == user_data["email"])
            todo = Todo(
                content=data["content"], user=user, posted_on=dt.datetime.utcnow()
            )
        return todo
...
def toggledone(pk):
    ...
    if errors:
        return jsonify(errors), 400
    return jsonify({"message": "Successfully toggled status.", "todo": data})
...
def new_todo():
    ...
    return jsonify(
        {"message": "Successfully created new todo item.", "todo": result.data}
    )
